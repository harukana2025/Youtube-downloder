...
body{background:linear-gradient(135deg,#0a0a2a,#1a1a4a);font-family:Segoe UI;min-height:100vh;padding:20px;animation:bgPulse 3s ease infinite}
@keyframes bgPulse{0%,100%{background:#0a0a2a}50%{background:#1a1a4a}}
.container{max-width:1000px;margin:0 auto}
.card{background:rgba(30,30,80,0.9);backdrop-filter:blur(10px);border-radius:28px;padding:30px;border:1px solid#4488ff;animation:slideUp 0.5s ease}
@keyframes slideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}
h1{text-align:center;color:#88aaff;font-size:2rem;margin-bottom:10px;animation:textGlow 2s ease infinite}
...
.mode-switch{display:flex;gap:15px;justify-content:center;margin-bottom:25px}
.mode-btn{padding:10px 25px;border-radius:40px;border:2px solid#4488ff;background:transparent;color:#aaaaff;cursor:pointer;transition:all 0.3s}
.mode-btn.active,.mode-btn:hover{background:#4488ff;color:white;transform:scale(1.05)}
.row{display:flex;gap:25px;flex-wrap:wrap}
.left{flex:1.5;min-width:280px}
.right{flex:1;min-width:240px}
input,select,button{width:100%;padding:12px 16px;margin:8px 0;border-radius:40px;border:2px solid#4488ff;background:rgba(20,20,60,0.8);color:white;font-size:14px}
input:focus{outline:none;border-color:#88aaff;box-shadow:0 0 10px#4488ff}
button{background:linear-gradient(135deg,#4488ff,#2244aa);cursor:pointer;border:none;transition:all 0.3s}
button:hover{transform:translateY(-2px);box-shadow:0 5px 15px#4488ff}
.info-panel{background:rgba(40,40,100,0.5);border-radius:16px;padding:15px;margin-top:15px;border:1px solid#4488ff;animation:fadeIn 0.5s}
@keyframes fadeIn{from{opacity:0}to{opacity:1}}
.info-item{display:flex;align-items:center;gap:10px;margin:8px 0;padding:5px}
.progress-area{margin-top:15px;display:none}
.progress-bar{height:20px;background:rgba(40,40,100,0.5);border-radius:10px;overflow:hidden}
.progress-fill{width:0%;height:100%;background:linear-gradient(90deg,#4488ff,#88aaff);transition:width 0.3s;animation:progressShine 1.5s infinite}
@keyframes progressShine{0%{opacity:0.7}50%{opacity:1}100%{opacity:0.7}}
.loading{display:inline-block;width:16px;height:16px;border:2px solid#4488ff;border-top-color:white;border-radius:50%;animation:spin 0.6s linear infinite;margin-right:8px}
...
<div id="thumbSetting" style="display:none"><select id="thumbSize"><option value="maxresdefault">最大</option><option value="hqdefault">高</option><option value="mqdefault">中</option><option value="default">標準</option></select></div>
...
document.getElementById('infoBtn').onclick=async function(){
    ...
    try{
        ...
        if(data.success){
            ...
            document.getElementById('infoPanel').innerHTML='<div class="info-item">📹 '+escapeHtml(data.title)+'</div><div class="info-item">👁️ '+formatNumber(data.view_count)+'</div>';
...
document.getElementById('saveBtn').onclick=async function(){
    ...
    pa.style.display='block';pf.style.width='0%';pm.innerHTML='準備中...';
    ...
    try{
        ...
        a.href=url;a.download={video:'video.mp4',audio:'audio.mp3',thumb:'thumbnail.jpg'}[currentMode];
        ...
        with yt_dlp.YoutubeDL(get_ydl_opts()) as ydl:
            ...
            for f in info.get('formats', []):
                if f.get('vcodec') != 'none' and f.get('height'):
                    formats.append({'itag': str(f['format_id']), 'height': f['height']})
            formats.sort(key=lambda x: -x['height'])
            return jsonify({'success': True, 'title': info.get('title'), 'view_count': info.get('view_count', 0), 'formats': formats, 'video_id': info.get('id')})
...
def download():
    ...
    try:
        if download_type == 'video':
            itag = data.get('itag')
            # 複数フォーマットを試行
            format_list = [f'{itag}+bestaudio/best', 'bestvideo+bestaudio/best', 'best', 'worst']
            for fmt in format_list:
                try:
                    output_template = os.path.join(temp_dir, 'video.%(ext)s')
                    ydl_opts = get_ydl_opts({'format': fmt, 'outtmpl': output_template, 'merge_output_format': 'mp4'})
            ...
            raise Exception('No format worked')
            ...
            for f in os.listdir(temp_dir):
                if f.endswith(f'.{audio_format}'):
                    return send_file(os.path.join(temp_dir, f), mimetype='audio/mpeg', as_attachment=True, download_name=f)
